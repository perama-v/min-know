...
#[test]
fn cid_extraction_2() {
    let test_metadata = "a165627a7a72305820deb4c2ccab3c2fdca32ab3f46728389c2fe2c165d5fafa07661e4e004f6c344a";
    let bytes = hex::decode(test_metadata).unwrap();
    let cid = cid_from_metadata(&bytes).unwrap().unwrap();
...
fn trims_metadata_2() {
    ...
    let bytes = hex::decode(getcode_result).unwrap();
    let trimmed = read_metadata(bytes.as_ref()).unwrap();
    let expected = "a165627a7a72305820deb4c2ccab3c2fdca32ab3f46728389c2fe2c165d5fafa07661e4e004f6c344a";
    assert_eq!(trimmed, hex::decode(expected).unwrap());
}
